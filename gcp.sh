...
API_IMAGE=blackboxdsp-api
WEB_IMAGE=blackboxdsp-web
DOMAIN=blackboxdsp.com

GCP_HOSTNAME=gcr.io
GCP_PLATFORM=managed
GCP_PROJECT_ID=blackboxdsp-311921
GCP_SERVICE_ACCOUNT=gcloud-api@blackboxdsp-311921.iam.gserviceaccount.com
GCP_AUTH_KEY_FILE=./conf/gcloud/gcloud-api.json
GCP_API_SERVICE=blackboxdsp-api
GCP_WEB_SERVICE=blackboxdsp-web
...
then
    ...
    gcloud run deploy "$GCP_API_SERVICE" --image="$GCP_API_IMAGE_PATH" --platform="$GCP_PLATFORM"
...
then
    ...
    gcloud run deploy "$GCP_WEB_SERVICE" --image="$GCP_WEB_IMAGE_PATH" --platform="$GCP_PLATFORM"
    echo -e "[Success]: Deployed service!\n"
